...
def name_extractor(text,terms,email_id,file_type,filename,phone):
    name_text = ""
    text  = re.sub("SUMMARY|PROFILE|Profile|EXPERIENCE|Experience|Summary|Contact|CONTACT|OVERVIEW|PROFESSIONAL|Professional|Overview"," ",text)
    terms = text.split("\n")
    name_list = []
    ...
    for i in range(len(name_list)):
        if not re.search(states,name_list[i]) and not re.search(job_titles,name_list[i].lower()) and not re.search("SUMMARY|PROFILE|Profile|EXPERIENCE|Experience|Summary|Contact|CONTACT|OVERVIEW|PROFESSIONAL|Professional|Overview", name_list[i]): 
