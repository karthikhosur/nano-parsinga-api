...
def name_extractor(text,terms,email_id,file_type):
    ...
    name_text = select_name_list(name_list)
    return name_text
...
def select_name_list(name_list):
    name_text = ""
    temp = ""
    terms = []
    ...
    if name_text =="":
        for i in range(len(terms)):
            temp = terms[i]
            temp = temp[:min(3,len(temp))]

            for j in range(len(terms)):
                if temp.lower() in terms[j].lower():
                    name_text = name_list[i]
                    break
            temp 
